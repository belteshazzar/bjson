...
/**
 * ObjectId class for MongoDB-compatible 24-character hex string (12 bytes)
 */
class ObjectId {
  constructor(value) {
    if (typeof value === 'string' && value.length === 24) {
      ...
      this.value = Array.from(value).map(b => b.toString(16).padStart(2, '0')).join('');
    } else {
      throw new Error('ObjectId must be a 24-character hex string or 12-byte Uint8Array');
    }
  }
